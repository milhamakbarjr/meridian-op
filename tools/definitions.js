...
- For single-side SOL deploys (amount_y only, amount_x=0), omit bins_above to use the
  automatic upside buffer from config (bins_above=0 if not configured). Do not use upside_pct.
...
WARNING: This executes a real on-chain transaction. Check DRY_RUN mode.`,
      parameters: {
        ...
        properties: {
          ...
          bins_above: {
            type: "number",
            description: "Number of bins above the current active bin. For single-side SOL deploys, omit to use the configured upside buffer. Only set explicitly for custom overrides."
          },
          downside_pct: {
            type: "number",
