...
const INTENT_TOOLS = {
  ...
  smartwallet: new Set(["add_smart_wallet", "remove_smart_wallet", "list_smart_wallets", "check_smart_wallets_on_pool", "refresh_smart_wallets"]),
...
const INTENT_PATTERNS = [
  ...
  { intent: "smartwallet", re: /\b(smart wallet|kol|whale|watch.?list|add wallet|remove wallet|list wallet|tracked wallet|check pool|who.?s in|wallets in|add to (smart|watch|kol)|refresh wallet|sweep wallet|find new wallet)\b/i },
...
export async function agentLoop(goal, maxSteps = config.llm.maxSteps, sessionHistory = [], agentType = "GENERAL", model = null, maxOutputTokens = null, options = {}) {
  ...
  for (let step = 0; step < maxSteps; step++) {
    ...
    try {
      ...
      const FALLBACK_MODEL = "deepseek/deepseek-chat-v3.1";
      let response;
